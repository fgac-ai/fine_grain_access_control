...
- **Missed the link in the chat?** If your agent asks for the same access again and you have not opened the link, FGAC emails it to your sign-up address from support@fgac.ai (subject starts with `Your agent has asked N times to …`). One email per request, one per agent turn even when the agent asked for several files at once, at most three a day, and you can reply to that email if you intentionally do not want the agent to have that access. FGAC never sends mail through your own Google account — the reminder comes from FGAC's own support mailbox, sent through FGAC itself.
- **Your agent keeps naming an account it cannot use?** A scheduled task that passes an `account` your profile does not include is refused every run, and there is no approval link for it (only you can add an account to a profile). After the third such refusal in a day, FGAC emails you once from support@fgac.ai naming the account the task passes and the accounts the profile can use, with the two fixes: change the task to use a listed account (or no `account` at all), or sign in as that other account and delegate it to you. One email per account per profile, ever; it counts toward the same three-a-day limit.
