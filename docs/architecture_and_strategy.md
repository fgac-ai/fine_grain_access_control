...
5. **Folders are out of reach under `drive.file`** (measured 2026-09-17, `docs/spike_results/drive-file-folder-pick.md`): picking a folder in the Picker grants the folder object only — `files.list` with `in parents` returns nothing and every child, existing or later-added, 404s. A folder grant or "read everything by default" therefore requires a restricted scope (`drive.readonly` / `drive`, annual CASA); the research plan `docs/implementation_plans/claude_great-dhawan-c05848_v5.md` recommends batch approvals within the Picker instead. Measured 2026-09-18 (`docs/spike_results/drive-scope-sheets-docs-edit.md`): the full `drive` scope alone reads and writes Sheets, Docs and Slides with no content-API scope, bounded by the user's own sharing permission — so a restricted-scope design would need only `drive` / `drive.readonly`, never `spreadsheets` / `documents` / `presentations`.
