...
4. **MCP Tools**: Sheets — `sheets_get_spreadsheet`, `sheets_read_range`, `sheets_update_range`, `sheets_append_rows`, `sheets_edit` (native batchUpdate requests: formatting, charts, tabs). Docs — `docs_read_document` (returns the raw Docs API resource; optional `fields` mask for large documents), `docs_edit` (native batchUpdate requests: text, tables, styles). Cross-service — `comments_read` / `comments_add` (Drive-API comments, enforced by the file's rule). Agent-created files are auto-granted read & write to the creating key — raw `POST v4/spreadsheets` / `POST v1/documents`, and since 2026-09-16 the Drive-side creates too (`POST drive/v3/files`, `POST drive/v3/files/{id}/copy`), because `drive.file` treats a file the app copies or uploads as app-owned exactly like one it creates natively; a copy additionally requires the source file to be exposed (any non-blocked rule) to the calling key.
5. **Folders are out of reach under `drive.file`** (measured 2026-09-17, `docs/spike_results/drive-file-folder-pick.md`): picking a folder in the Picker grants the folder object only — `files.list` with `in parents` returns nothing and every child, existing or later-added, 404s. A folder grant or "read everything by default" therefore requires a restricted scope (`drive.readonly` / `drive`, annual CASA); the research plan `docs/implementation_plans/claude_great-dhawan-c05848_v8.md` — decision 2026-09-19: move to the full `drive` scope with folder-inherited rules (plan v7); batch approvals stay as an interim. Measured 2026-09-18 (`docs/spike_results/drive-scope-sheets-docs-edit.md`): the full `drive` scope alone reads and writes Sheets, Docs and Slides with no content-API scope, bounded by the user's own sharing permission — so a restricted-scope design would need only `drive` / `drive.readonly`, never `spreadsheets` / `documents` / `presentations`.
6. **Kind descriptor**: everything per-file (rules, Picker exposure, grant verification, recovery pages, approval links) keys off `src/lib/driveFileKinds.ts`. Adding the next file type (Slides is stubbed) means a descriptor entry + tool definitions + QA docs — shared code must not grow `if (kind === ...)` branches.
