# Full Drive / folder support — research plan v1

Branch: `claude/great-dhawan-c05848` · Date: 2026-09-17 · Status: research, no code change
...
files in one go). Reject Design B (folder grant under `drive.file`) unless the
folder-pick experiment in §1.1 shows children are reachable — as of this
revision the experiment result is recorded in §1.1. Defer Design C
...
> RESULT PENDING AT THIS REVISION — filled in by v2 with the request/response
> table (ids redacted). The design section below states which designs each
> outcome unlocks so the recommendation can be read either way.
...
### Design B — folder grant via the Picker (conditional on §1.1)

Only viable if the folder pick reaches children. Then: a `folder_read` /
`folder_read_write` rule (`service: 'drive'`, `targetResourceId = folderId`),
resolution by `files.get(fileId, fields=parents)` cached per file, a FOLDERS
view with `setSelectFolderEnabled(true)` on the approve page and dashboard,
and "late children" semantics documented from the experiment. Cost: ~2 weeks,
no audit, still `drive.file`. If the pick grants only the folder object, this
design is dead under `drive.file` and its folder semantics move to Design C.
...
Accept **A** now; run the §1.1-driven decision on **B** in the same PR cycle
(it is cheap if the measurement allows it); put **C** on a dated review after
...
| Code | Design A items 1–4; the §1.1 spike behind a flag only if children are reachable; monitoring 7.14/7.19 already cover the success measure, add the per-person distribution query (§5). |
