...
| QA | capability 16 A31 |

## Validation plan

1. `npx tsx scripts/test-clerk-auth-redirect.ts`, `tsc`, `eslint`,
   `npx tsx scripts/qa-coverage-check.ts` (doc parses; A31 counted).
...
## QA outcome (capability 16 A31, 2026-09-24)
...
(A31 `blocked` with the user action named; A1–A29 `skip`, out of scope).
