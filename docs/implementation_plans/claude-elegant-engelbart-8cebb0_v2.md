# First Grok install: attribute the `connectors-manager` client family — plan v2 (validated on the preview)
...
- HogQL over 30 days: the new match strings hit only the 2026-09-24 Grok and
  Cursor rows.
- Preview (PR #165, deployment `cw07f6qnh`, commit `64d0301`, confirmed via
  `vercel ls --meta githubCommitSha`): three tokenless `initialize` requests
  shaped as `connectors-manager`, `grok-validator` (both on
  `grok-connectors-manager/0.1.0`) and `Cursor` on `Cursor/1.0.0` each took
  the expected 401 and landed in PostHog (environment `preview`,
  2026-09-25T12:59Z) as `mcp_auth_attempt` + `connector_install_started`
  with `client_class = 'direct'` and `client_class_signal = 'product:grok'`
  / `'product:cursor'` — including `grok-validator`, which production had
  labelled `scanner`. Ingestion lagged about a minute; the first query saw
  one row of six.
- After the production deploy: 7.21f-b shows `grok` on the next Grok
  handshake; 7.21e's `unlabelled` column no longer grows with Grok traffic.
