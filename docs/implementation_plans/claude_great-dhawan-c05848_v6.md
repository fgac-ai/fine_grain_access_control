# Full Drive / folder support — research plan v6

Branch: `claude/great-dhawan-c05848` · Date: 2026-09-19 · Status: research, no code change · Supersedes v5 (corrects a stale claim: the production Slides API is enabled; Slides calls succeed in production since 2026-09-17)
...
sound — measured 2026-09-18, one scope reads and writes Sheets, Docs and
Slides with no content-API scope — but it turns
...
**Yes on both counts — measured on 2026-09-18** (`docs/spike_results/drive-scope-sheets-docs-edit.md`).
With USER_A's grant widened to `drive` and tokeninfo showing no Sheets, Docs
or Slides scope, the runner listed a folder's four children, read and wrote a
Sheet (cell update, add and delete a tab), read and wrote a Doc (insertText
verified in the body), and created a slide in a Slides deck; a shared Sheet
where USER_A is a viewer read 200 and wrote 403 `PERMISSION_DENIED`, and
whole-Drive listing worked. Every write was reverted and the grant restored
to `drive.file` + `gmail.modify`. The documentation agrees: Google's
per-method scope tables list
...
needs nothing extra either: production Slides calls have succeeded since the
Slides release on 2026-09-17 (the `SERVICE_DISABLED` 403s that preceded it
ended on 2026-09-16, per production `$mcp_tool_call` data queried 2026-09-19).
...
### 1.6 The spike itself, and what it taught about incremental consent

Ken asked (2026-09-17) for a real spike: widen a QA account's grant to the
full `drive` scope and try to edit Sheets and Docs with it. Attempt 1
(2026-09-17) stalled on Clerk's callback in the built-in browser; attempt 2
(2026-09-18, Path B Chrome) completed end to end and is what §1.5 reports.
Two things measured on the way matter for Design C:
...
- Clerk's OAuth callback failed in the built-in browser (`authorization_invalid`
  ...
  Chrome; the QA harness runs this leg through Path B.
- Restoring the narrow grant is a two-step routine that works: Google account
  permissions → Remove all access for the dev app, then a plain reauthorize
  with `prompt=consent`; the external account stayed `verified` throughout,
  so the destroy-and-recreate branch was never needed.

Final state: USER_A holds exactly `drive.file` + `gmail.modify` again on a
fresh refresh token; probe files remain in its Drive, clean.
