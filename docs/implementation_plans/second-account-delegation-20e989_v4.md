...
> Branch: `claude/zealous-bassi-20e989` — v4 (2026-09-22, validated; PR #158)
>
> **v4 changes:** round 3 still saw the wall's done wording flip ~1.1 s after the
> click in 1 of 3 runs — the page re-renders after the action even with no
> revalidation — so the panel now keys its done view on client state
> (`confirmedHere`), never on the server prop; round 4 held 3/3 timed runs.
> Preview (commit 7c3cbf9): 04 A9, 04 A10 (both directions), 14 A19, 14 A5 all
> pass. A first preview run reported the minted link as "Invalid link" for the
> owner and blamed the mint script's local-branch lookup; a read-only compare of
> the two Neon branches showed identical users rows and keys, both environments
> share the Clerk secret, and the re-run with step-by-step diagnostics (owner
> uuid = delegate link uuid, URL byte-identical, owner open renders the approve
> flow) could not reproduce it — a runner mis-step, not an environment gap. The
> mint script now prints `owner_id` / `proxy_key_id` so that check is one line.
>
> **v3 changes:** round 2 passed the switch direction (both directions), the
> one-offer-at-a-time landing and the A5 regression, but the wall's done state
> was STILL replaced — at ~1.2 s instead of ~250 ms. Root cause, established
> by the runner's millisecond polling: any `revalidatePath`/`revalidateTag`
> inside a server action makes Next re-render the CURRENT route in the action
> response, whatever path was named, and the server tree swapped the client
> panel for a static "already attached" card, unmounting it. Two changes:
> (1) `delegateToApprovalOwner` and the banner path of `delegateToUser` no
> longer revalidate at all (dynamic dashboard pages refetch on their next
> navigation; only the Accounts-page landing keeps `revalidatePath` because
> its "Delegations You've Granted" list is on the same page); (2) every surface
> renders the SAME `DelegateToPanel` at the same tree position in both states
> (`initialDone`), so a re-render updates props instead of unmounting the
> client's done state, and a return visit renders the done view server-side
> ("… is already attached …" with the Switch button). The old
> `delegate-panel-active` and `delegate-link-notice[data-state=already_active]`
> are gone; QA docs updated. Round 3 (local) verifies persistence over a 5 s
> poll on the wall and the banner; the preview pass runs the full scope.
>
