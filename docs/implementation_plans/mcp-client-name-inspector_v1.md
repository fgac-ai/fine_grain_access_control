...
Established before building (queries in `docs/monitoring.md` 7.31):
...
| backfill script over production rows | **rejected** | rows self-heal: all 65 mislabelled callers handshook as ClaudeAI in the same week (about 18 times each); rows whose owner never returns generate no tool calls, so nothing stays mislabelled. Historical events are read with the 7.31 fold instead |
...
- Docs: `monitoring.md` 7.31 (window, reading rule, three queries),
...
- Preview: a bearer minted on the preview deployment, then the measured
  ...
  7.31a is the production check a week after the deploy.
