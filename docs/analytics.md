...
| `sales_lead_emailed` | server (`/api/sales-lead`, `src/lib/salesLead.ts`) | `status` (`sent` / `disabled` / `failed`), `source` (`pricing`), `signed_in`, `team_size`, `needs`, `has_company`, `error` (failed only). One per Send: the submitter is emailed a confirmation carrying what they entered, **Cc the sales inbox, Reply-To sales**, from the support sender through FGAC's own proxy (same path as `approval_link_notified`). `disabled` = `SUPPORT_FGAC_PROXY_KEY` / `SUPPORT_SENDER_EMAIL` unset — true on local and preview (they are Production-only), so the form there says "we'll reply" instead of "we've emailed". `SALES_SENDER_EMAIL` optionally overrides From (must be a send-as alias of the support mailbox). Distinct id = the Clerk user id when signed in, else `anonymous-sales-lead`. Honeypot hits are dropped before this fires |
