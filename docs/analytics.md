...
| `sign_up_started` | client (`SignUpCta.tsx`, all sign-up CTAs) | `cta_location`: nav / hero / bottom_cta / pricing_free |
...
| `pricing_interest_submitted` | client (`PricingPlans.tsx`, the Pro door) | `plan` (`pro`; v1/v2 also had `personal` / `team`) — **`signed_in: true` is the willingness-to-pay count**: an existing user pressed Upgrade to Pro and then "Count me in" on a dialog that names the price — `interval`, `signed_in`, `pricing_variant` (`team_size` existed on v2's Team door only). Also sets person properties `pricing_interest_plan`, `pricing_interest_interval`, `pricing_interest_at`, and — for signed-out submitters only — `email` (signed-in persons already carry it). The launch list is therefore `SELECT properties.email, properties.pricing_interest_plan FROM persons WHERE properties.pricing_interest_plan IS NOT NULL`; nothing is written to our database |
