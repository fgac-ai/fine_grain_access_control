...
| `pricing_interval_toggled` | client (`/pricing`, `PricingPlans.tsx`) | `interval` (`monthly` / `annual`), `pricing_variant`. The pricing page (added 2026-09-17) is a **fake door**: prices are shown, nothing is billed or enforced. It exists to measure which plan prospective users reach for, so every interaction is captured. Rationale and read-out order: `docs/implementation_plans/pricing-page-design-9dbc83_v1.md` |
| `pricing_plan_clicked` | client (`PricingPlans.tsx`, `EnterpriseCta.tsx`) | `plan` (`personal` / `pro` / `team` / `enterprise`), `interval` (`monthly` / `annual`, or `contract` for enterprise), `signed_in`, `pricing_variant`. Fires on every plan CTA before anything else happens, so click-through by plan is measurable even when the door is then dismissed. `personal` also fires the ordinary `sign_up_started {cta_location: 'pricing_personal'}` for signed-out visitors |
| `pricing_interest_submitted` | client (`PricingPlans.tsx`, the fake-door dialog) | `plan` (`pro` / `team`), `interval`, `signed_in`, `team_size` (team only: `2–5` / `6–20` / `21–100` / `100+` / `unspecified`), `pricing_variant`. Also sets person properties `pricing_interest_plan`, `pricing_interest_interval`, `pricing_interest_at`, `pricing_interest_team_size`, and — for signed-out submitters only — `email` (signed-in persons already carry it). The launch list is therefore `SELECT properties.email, properties.pricing_interest_plan FROM persons WHERE properties.pricing_interest_plan IS NOT NULL`; nothing is written to our database |
