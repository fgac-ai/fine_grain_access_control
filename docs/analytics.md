...
| `pricing_plan_clicked` | client (`PricingPlans.tsx`) | `plan` (`free` / `pro` / `enterprise`; v1/v2 used `personal` / `team`), `interval` (`monthly` / `annual`, or `contract` for enterprise), `signed_in`, `pricing_variant`. Fires on every plan CTA before anything else happens, so click-through by plan is measurable even when the door is then dismissed. A signed-out `free` click is a real sign-up and also fires the ordinary `sign_up_started {cta_location: 'pricing_free'}`; `pro` opens the door; `enterprise` is a `mailto:sales@fgac.ai` link |
| `pricing_interest_submitted` | client (`PricingPlans.tsx`, the Pro door; signed-in only since 2026-09-20 — signed-out visitors get a sign-up button instead of an email field) | `plan` (`pro`; v1/v2 also had `personal` / `team`), `interval`, `signed_in` (always `true` now), `pricing_variant`. **This is the willingness-to-pay count**: an existing user pressed Upgrade to Pro and then "Count me in" on a dialog that names the price. Also sets person properties `pricing_interest_plan`, `pricing_interest_interval`, `pricing_interest_at` on the identified person (their `email` is already there from `sign_up_completed`), so the launch list is `SELECT properties.email FROM persons WHERE properties.pricing_interest_plan IS NOT NULL`; nothing is written to our database |
