...
| `approval_link_notified` | server (`src/lib/approvalNotify.ts`, from every mint path) | `channel` (`email`), `action`, `request_id`, `link_count` (2 for a send denial: the per-recipient and send-to-anyone links ride in one email). Fires once per request, ever — the first mint of a request emails its link to the OWNER's own inbox through the owner's own Gmail grant, claimed atomically on `approval_requests.notified_at` before the send. Why (measured 14 d to 2026-09-13): ~half of minted requests were never opened, and the never-openers cluster on agent surfaces that collapse the tool result (Claude Code) — the person never sees the URL however the denial is worded. Not a funnel stage: join it to `approval_link_minted` on `request_id` to split the funnel by delivery (`notify_status` on the mint carries the same fact plus the skip reasons). Runbook: `monitoring.md` 7.23 |
