...
| `google_grant_dead_notified` | server (`src/lib/approvalNotify.ts` `notifyOwnerOfDeadGrant`, from the `google_token_unavailable` refusal in `resolveAccountAndToken`) | `channel` (`email`), `trigger` (`first` / `repeat`), `reason` (`grant_revoked` / `refresh_failed` / `no_token` — the reconnect-repairable classes of `google_token_fetch_failed`), `account_delegated`, `cc_delegate` (the key owner was CC'd — true exactly when delegated), `notice_number` (1–3), `max_notices` (3), `failure_count` (refusals in the current episode), `days_dead` (whole days since the episode's first refusal), `via` (`mcp`). **Captured for the mailbox OWNER** (the recipient — for a delegated mailbox that is the grantor, not the person whose agent was refused), so `uniq(person)` is the notified population and the row joins to the owner's own `google_reconnect_*` funnel. Fires when an agent is refused because the mailbox's Google grant is dead and a reconnect would repair it: on the FIRST such refusal of an episode, then again only if it is still failing 7 days after the previous notice, at most 3 notices per episode (a failure more than 14 days after the previous one starts a new episode). Same sender, transport and 3-per-person daily cap as `approval_link_notified` (all three ledgers count). Why: in the 30 d to 2026-09-19 one delegated mailbox was refused once a day for 30 days with zero successes and its owner never told (the agent's reconnect link is bound to the owner, and the delegate cannot use it), and two own-mailbox owners got exactly ONE refusal each, never opened the dashboard, and were still dead 48 h later — a second-failure rule would have reached neither, hence first-failure. The three owners who did recover all did so after visiting the dashboard (9 h – 4 d). Never fires from the quiet `list_accounts` probes, from the proxy path (0 failures in 30 d) or for `owner_not_found` / `delegation_inactive` / transient `clerk_error` / `timeout` (nothing to reconnect, or the retry covers it). Ledger: `google_grant_failures` (one row per owner + mailbox; `notified_at` / `notified_count` are the claim). Runbook: `monitoring.md` 7.29 |
