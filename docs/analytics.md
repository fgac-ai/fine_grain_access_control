...
| `sign_up_started` | client (`SignUpCta.tsx`, all sign-up CTAs) | `cta_location`: nav / hero / bottom_cta / pricing_personal |
...
| `pricing_interval_toggled` | client (`/pricing`, `PricingPlans.tsx`) | `interval` (`monthly` / `annual`), `pricing_variant`. The pricing page (added 2026-09-17) is a **fake door**: prices are shown, nothing is billed or enforced. It exists to measure whether people will pay, so every interaction is captured. `pricing_variant` is `v1-2026-09` for the three-tier draft Ken rejected the same day and `v2-2026-09` for the shipped mechanic (one flat Personal plan after a 30-day trial, Team per seat). Strategy, comparables and read-out order: `docs/implementation_plans/pricing-page-design-9dbc83_v2.md` |
| `pricing_plan_clicked` | client (`PricingPlans.tsx`, `EnterpriseCta.tsx`) | `plan` (`personal` / `team` / `enterprise`; v1 also had `pro`), `interval` (`monthly` / `annual`, or `contract` for enterprise), `signed_in`, `pricing_variant`. Fires on every plan CTA before anything else happens, so click-through by plan is measurable even when the door is then dismissed. A signed-out `personal` click is a real sign-up (the trial is today's product) and also fires the ordinary `sign_up_started {cta_location: 'pricing_personal'}`; a signed-in `personal` click is the **Subscribe** button and opens the door |
| `pricing_interest_submitted` | client (`PricingPlans.tsx`, the fake-door dialog) | `plan` (`personal` / `team`; v1 had `pro`) — **`personal` with `signed_in: true` is the willingness-to-pay count**: an existing user pressed Subscribe and then "Count me in" on a dialog that names the price — `interval`, `signed_in`, `team_size` (team only: `2–5` / `6–20` / `21–100` / `100+` / `unspecified`), `pricing_variant`. Also sets person properties `pricing_interest_plan`, `pricing_interest_interval`, `pricing_interest_at`, `pricing_interest_team_size`, and — for signed-out submitters only — `email` (signed-in persons already carry it). The launch list is therefore `SELECT properties.email, properties.pricing_interest_plan FROM persons WHERE properties.pricing_interest_plan IS NOT NULL`; nothing is written to our database |
