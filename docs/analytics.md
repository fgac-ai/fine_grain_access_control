...
| `sign_up_started` | client (`SignUpCta.tsx`, all sign-up CTAs) | `cta_location`: nav / hero / bottom_cta / pricing_free / pricing_pro (the sign-up button inside the Pro dialog for signed-out visitors) |
...
| `pricing_interval_toggled` | client (`/pricing`, `PricingPlans.tsx`) | `interval` (`monthly` / `annual`), `pricing_variant`. The pricing page (added 2026-09-17) is a **fake door**: prices are shown, nothing is billed or enforced. It exists to measure whether people will pay, so every interaction is captured. `pricing_variant` is `v1-2026-09` for the three-tier per-account draft and `v2-2026-09` for the trial-then-paid draft (both rejected in review, neither reached production) and `v3-2026-09` for the shipped mechanic: Free capped at 50 successful requests/month, Pro $5/month or $30/year per person, Enterprise by contact-sales. Decision and cap analysis: `docs/implementation_plans/pricing-page-design-9dbc83_v3.md`; cost base and comparables: `_v2.md` |
| `pricing_plan_clicked` | client (`PricingPlans.tsx`) | `plan` (`free` / `pro` / `enterprise`; v1/v2 used `personal` / `team`), `interval` (`monthly` / `annual`, or `contract` for enterprise), `signed_in`, `pricing_variant`. Fires on every plan CTA before anything else happens, so click-through by plan is measurable even when the door is then dismissed. A signed-out `free` click is a real sign-up and also fires the ordinary `sign_up_started {cta_location: 'pricing_free'}`; `pro` opens the Pro door; `enterprise` opens the Contact-sales form |
| `pricing_interest_submitted` | client (`PricingPlans.tsx`: the Pro door and, since 2026-09-21, the Contact-sales form) | `plan` (`pro` / `enterprise`; v1/v2 also had `personal` / `team`), `interval` (`monthly` / `annual`, or `contract` for enterprise), `signed_in`, `pricing_variant`; enterprise rows add `team_size` (`2–10` / `11–50` / `51–250` / `250+` / `unspecified`), `company` (free text, optional) and `needs` (array from BAA / SOC 2 report / SSO / SAML / Self-hosted / DPA / security review / Invoicing). **`plan = 'pro'` is the willingness-to-pay count** (signed-in only: an existing user pressed Upgrade to Pro and then "Count me in" on a dialog that names the price; signed-out visitors get a sign-up button instead). **`plan = 'enterprise'` is a sales lead** — the Contact sales button opens a form (a bare `mailto:` did nothing visible in browsers without a mail handler, 2026-09-21) whose submission lands here; the sales mailbox is offered as a fallback link only, so until a server-side email to sales is wired, **leads exist only in PostHog**. Both set person properties `pricing_interest_plan`, `pricing_interest_at` (Pro also `pricing_interest_interval`; enterprise also `pricing_interest_team_size`, `pricing_interest_company`, and `email` for signed-out submitters), so the launch list and the lead list are `SELECT properties.email, properties.pricing_interest_plan, properties.pricing_interest_company FROM persons WHERE properties.pricing_interest_plan IS NOT NULL`; nothing is written to our database |
| `pricing_faq_opened` | **retired 2026-09-20** (the FAQ section was removed from `/pricing` before it ever reached production; no production rows exist) | was: `question`, `pricing_variant` |
| `pricing_sales_form_field` | client (`PricingPlans.tsx`, the Contact-sales form) | `field` (`email` / `company` / `team_size` / `needs`), `value` (the typed or chosen value — Typeform-style partial capture, Ken 2026-09-21: a lead who never presses Send is still a lead), `signed_in`, `pricing_variant`. Text fields fire once per typing pause (700 ms), choices at once. A plausible typed email is also set as the person's `email` (with `pricing_interest_plan: 'enterprise'`) so partial leads are reachable |
| `pricing_sales_form_abandoned` | client (`PricingPlans.tsx`) | `fields_filled` (subset of `email` / `company` / `team_size` / `needs`), `seconds_open`, `signed_in`, `pricing_variant`. Fires when the Contact-sales dialog is closed without Send (Escape, backdrop, Not now, ✕). Opened − submitted − abandoned should be ~0 |
| `sales_lead_emailed` | server (`/api/sales-lead`, `src/lib/salesLead.ts`) | `status` (`sent` / `disabled` / `failed`), `source` (`pricing`), `signed_in`, `team_size`, `needs`, `has_company`, `error` (failed only). One per Send: the submitter is emailed a confirmation carrying what they entered, **From the sales alias, Cc the sales inbox, Reply-To sales**, sent through the support mailbox's proxy key (the alias belongs to that mailbox) exactly like `approval_link_notified`. `disabled` = `SUPPORT_FGAC_PROXY_KEY` / `SUPPORT_SENDER_EMAIL` unset — true on local and preview (they are Production-only), so the form there says "we'll reply" instead of "we've emailed". `SALES_SENDER_EMAIL` optionally overrides From (must be another alias of the support mailbox). Distinct id = the Clerk user id when signed in, else `anonymous-sales-lead`. Honeypot hits are dropped before this fires |
...
| `sign_in_completed` | client (`SignInTelemetry.tsx`, dashboard pages) | `gmail_scope`, `drive_file_scope`, `needs_drive_file` (user has Sheets/Docs rules), `drive_file_narrowed` (= needs it and arrived without it). Once per Clerk `lastSignInAt` (localStorage-keyed; fires only within 10 min of the sign-in). Added 2026-09-04: PostHog had no sign-in signal, and a plain Google sign-in rewrites Clerk's grant with the sign-in scope set — no `drive.file` — so `drive_file_narrowed` is the per-sign-in count of users whose Sheets/Docs access a sign-in just broke. Detection query: monitoring.md §7.12. Since 2026-09-16 also `client` (`browser` / `claude_desktop`, same classifier as `approval_link_opened`), `landing_path`, and `after_approval_wall` — true when the `fgac_approval_wall` marker cookie set by the approval sign-in wall (below) was present, with `approval_wall_action`, `approval_wall_target_hash`, `approval_wall_age_s`. The component never mounts on `/dashboard/approve`, so `after_approval_wall: true` IS the lost-context sign-in: the person hit the wall, signed in, and landed on a dashboard page instead of the approval. Runbook: monitoring.md §7.25 |
| `video_played` | client (`TrackedVideoEmbed.tsx`, all Descript demo embeds) | `video_id`, `video_title`, `page` |
| `$mcp_tool_call` | server (`/api/mcp`, every tool) | `$mcp_tool_name`, `$mcp_duration_ms`, `$mcp_is_error`, `client_id`, `client_name`, `user_agent`, `outcome`, `account_email`, `account_delegated`; raw tools add `raw_api_kind`, `raw_api_family`, `raw_api_endpoint`, `raw_api_mutating`; denials add `denial_code`, and denials carrying an approval link add `approval_request_id` (joins to the approval funnel); caller-chosen `account_not_permitted` refusals (the agent passed an `account` the key cannot use — no link is minted) add `account_requested` (the refused value, lower-cased; until 2026-09-16 it was recorded nowhere), `account_refusal_count` (refusals of that value on that key in the current rolling 24 h), `notify_status` (`sent` / `already_sent` / `not_due` / `skipped_episode` (the owner was emailed about a different refused value within 14 d) / `skipped_placeholder` (the value cannot be anyone's mailbox — never ledgered, never emailed) / `skipped_rate_capped` / `failed` / `disabled` — whether THIS refusal emailed the owner; see `account_refusal_notified`) and, since 2026-09-24, `account_requested_placeholder` (`reserved_domain` (RFC 2606/6761: example.com, .test, .invalid, .localhost…) / `synthetic` (a `user@` local part on `domain.com`, `john.doe@…`) / `malformed` (not an address) — absent when the value could be a real mailbox; `src/lib/placeholderEmail.ts`. One Claude.ai agent re-sent two invented example.com addresses 140 times in three days, 2026-09-21 → 23, and the generic refusal earned its owner two emails about accounts nobody has; docs/monitoring.md 7.22); `google_token_unavailable` denials whose class a reconnect repairs (`no_token` / `refresh_failed` / `grant_revoked`) add `notify_status` too (whether THIS refusal emailed the mailbox owner — `sent` / `already_sent` / `skipped_rate_capped` / `skipped_global_capped` (the 10-per-hour breaker across all owners tripped) / `failed` / `disabled`; see `google_grant_dead_notified`), `grant_failure_count` (refusals on that owner + mailbox in the current episode) and `grant_days_dead` (whole days since the episode began — the 30-day delegated case reads `30` here); since 2026-09-25 the scope pre-flight refusals (`denial_code` `gmail_scope_missing` / `drive_file_scope_missing`) carry the same three properties — the same ledger and notice, keyed on the mailbox owner, one email per (owner, mailbox) episode, a class change (dead ↔ scope-missing) starting a new episode (docs/monitoring.md 7.30e); failures add `error_status`, `error_reason`, `error_domain`, `failure_reason`, `gmail_404_site`; Google 400s add `bad_request_kind` (`range_parse` / `grid_limits` / `values_overflow` / `values_shape` / `request_index` / `fields_mask` / `unknown` — FGAC's classification of what Google objected to, docs/monitoring.md 7.28), `bad_request_index` when Google named a batchUpdate `requests[N]`, and sheets range errors add `sheet_tabs_listed` (how many tabs the error text listed; `0` = the metadata read failed and the text fell back to pointing at `sheets_get_spreadsheet`); 404s on a key that reaches more than one mailbox add `cross_mailbox_hint: true` (the response listed the other mailboxes and the `account:` fix — docs/monitoring.md 7.23); per-file sheets/docs/slides calls add `file_id` + `file_service` (`sheets` / `docs` / `slides`; stamped in `checkFilePermission`, so every typed sheets/docs/slides tool, comments tool, and raw per-file API call carries them — denied outcomes included — making per-file time-to-first-success queryable and joining to `rule_saved.file_id`); `gmail_get_attachment` adds `attachment_selector` and, on 404 recovery paths, `attachment_selfheal`; windowed reads (`gmail_get_attachment`, `gmail_read`, `docs_read_document`, `slides_get_presentation`, `sheets_get_spreadsheet`, `sheets_read_range`, `google_api_get`) add `window_offset`, `window_chars`, `window_total_chars`; calls that reach Google add `google_ms` (cumulative wall-clock inside `googleFetch`) and `token_ms` (Clerk token fetch); calls whose Clerk token fetch failed on the first attempt add `google_token_retry` (`recovered` = the single server-side retry got the token and the call went on normally; `retry_failed` = it did not, and the event also carries `google_token_error`) Since 2026-09-23: `arg_aliases` / `arg_alias_targets` / `arg_alias_count` when the transport wrapper renamed argument keys the agent sent (`spreadsheet_id` → `spreadsheetId`, `url` → `path`, …) before the SDK validated the call — see `mcpArgumentGuidance.ts`; absent on calls that needed no aliasing. Runbook 7.10a. |
...
| `mcp_connection_client_identified` | server (`/api/mcp` auth layer, name-on-touch) | `connection_id`, `client_id`, `client_name`, `client_version`, and since PR #162 `transition` (`first` — the opaque `client_id` placeholder replaced by a name, `inspector_to_product` — the directory's connect-time `Anthropic/Toolbox` replaced by the real client, `product_switch` — a registration shared between claude.ai and Claude Code changing hands) and `previous_client_name`. Fires on **every name change** (before PR #162: once, on the first name, which for directory connections was the inspector). The FIRST event per connection is how it arrived; the LATEST is the product in use — join on `connection_id`, see `monitoring.md` 7.32 |
...
| `delegation_created` | server (dashboard action) | `delegate_email`, `reactivated`; since 2026-09-21 `via` — `form` (the typed-email "Delegate Access" form), `approve_wall` (the wrong-account approval card's "attach this mailbox to the owner's account"), `accounts_link` (someone opened `/dashboard/accounts?delegate_to=<id>` — the "+ Add account" link — and confirmed), `dashboard_banner` (the fresh account whose browser held another FGAC session minutes ago said "yes, that's me"); `approve_wall` rows add `prior_session_matches` and `action`, `dashboard_banner` rows `prior_gap_s`. Conversion per surface = `delegation_created{via}` / `delegation_prompt_shown{surface}` per person (monitoring §7.29) |
| `delegation_prompt_shown` | server (approve page, Accounts page, dashboard pages) | `surface` (`approve_wall` / `accounts_link` / `dashboard_banner`), plus per surface: `prior_session_matches` + `action` + `request_id` (wall — the browser held the link owner's session just before this account, per the `fgac_last_account` / `fgac_prev_account` markers middleware stamps on every signed-in dashboard request), `state` (`offer` / `self` / `already_active` / `missing` — what the delegate link rendered) + `account_age_s` (link), `direction` (`delegate` — the signed-in account is the newer one and can grant the delegation here; `switch` — it is the older, primary one, so the prompt offers to sign out and confirm as the newer account via the delegate link) + `prior_gap_s` + `since_switch_s` + `account_age_s` (banner). Added 2026-09-21 for the second-FGAC-account repair (src/lib/secondAccount.ts): three people in one week created a second account and got stuck between the two. Server-rendered, so renders overcount views — read people as `uniq(distinct_id)` |
| `delegation_prompt_dismissed` | client (`DelegateToPanel.tsx`) | `surface`. The dashboard prompt's "No, that's someone else" (also retires the marker cookie) |
| `delegation_link_copied` / `delegation_link_switch_clicked` | client (`AddDelegatedAccountButton.tsx`) | `surface: 'add_account_dialog'`. Since 2026-09-21 the "+ Add account" dialog hands out the delegate link instead of an explanation; `switch` = signed out and returned to the link in the same browser. The old dialog's "Got it" `$autocapture` rows end at that deploy |
| `account_linked` | server (dashboard action) | `target_email`, `delegated`, `via` |
| `agent_profile_created` | server (dashboard action `createProxyKey`) | `accounts` (addresses attached at creation), `delegated_accounts`, `existing_profiles` (live keys before this one, Default Profile included — `1` is a user's first non-default profile). The label is never captured. Fires only when the key row is written. No PostHog event carries a key id, so profile adoption itself (bound connections, use, rules) is read from the database with `npm run profiles:usage -- --prod` (`docs/monitoring.md` 7.24) |
| `agent_profile_create_failed` | server (dashboard action `createProxyKey`) | `accounts`, `reason` (`missing_label` / `unslugifiable_label` / `slug_clash` / `no_delegation` — every refusal the action returns as a value; all four are by design and the dialog shows the message), `existing_profiles` where known. The "trying and failing" half of the daily review's AGENT PROFILES section: a person with this event (or a `+ New profile` click) and no `agent_profile_created` in the same session is an abandoned attempt |
| `approval_link_minted` | server (`/api/mcp` — policy denial, send denial, `request_access`) | `action`, `request_id`, `target_hash`, `mint_count`, `via` (`send_denial`/`request_access`; absent for policy denials), since 2026-09-15 `notify_status` (`sent` / `already_sent` / `not_due` / `skipped_opened` / `skipped_burst` / `skipped_rate_capped` / `failed` / `disabled` — whether THIS mint triggered the reminder email from FGAC's support mailbox; `not_due` = first ask or inside the 5-minute same-turn window; `skipped_burst` (since 2026-09-24) = due, but ANOTHER request of this owner was emailed inside that same window — one email per agent turn, this link stays eligible for a later turn; see `approval_link_notified`), and on `request_access` mints since 2026-09-08 `has_resource_name` (the request has a title — passed on this call or stored by an earlier one in `approval_requests.resource_name`, shown on the approve page: the only name source for a file Google does not share with FGAC yet) and `resource_name_supplied` (this call passed one). **Fires once per mint ATTEMPT**, so `uniq(request_id)` is demand and `count()` is retry pressure |
| `approval_link_opened` | server (approve-page load, `/dashboard/approve`) | `status` (`fresh`/`already_granted`/`wrong_account`/`invalid`), `request_id` (real id for `wrong_account` — recomputed against the resolved owner; `undefined` only for `invalid`), `action`, `agent_driven`, `user_agent`; since 2026-09-09 `client` (`browser` / `claude_desktop` / `agent`, from `src/lib/approveClientClass.ts`). Since 2026-09-15 `link_source` (`email` when the URL carried the reminder email's `src=email` marker, `banner` since 2026-09-20 when the dashboard's pending-approvals banner opened it (`src=banner`), else `agent`) — the marker is outside the signed params and survives the page's sign-out / try-again round trips, so it changes nothing about verification and stays attributed through recovery. **`claude_desktop` is a person** — the Claude desktop app's in-app browser sends a Chrome UA with a `Claude/<build>` token, and before this the bare "claude" agent test counted those opens as `agent_driven: true` (19 opens / 7 people in the 30 days to 2026-09-09, every one followed by that person's own pageviews and pick-button clicks). Read human reach as `client != 'agent'`; `agent_driven` on older rows overstates agents by exactly this class. Fires on every server render of the page (a route refresh or navigation re-renders it — local QA 2026-09-09 saw extra rows while the tab sat idle), so rows overcount opens: always read `uniq(request_id)`. Since 2026-09-21 `wrong_account` rows also carry `delegate_offer` (the card offered "attach this mailbox to the owner's account"), `delegation_active` (the visitor's mailbox is already delegated to the owner — repair done, only the switch left) and `prior_session_matches` (this browser held the owner's session just before, so the offer led the card) |
| `approval_sign_in_wall` | edge middleware (`src/middleware.ts` → `src/lib/approvalWall.ts`, `src/lib/posthogEdge.ts`) | `action`, `proxy_key_id`, `target_hash` (same HMAC as the mint/open events — the join key: `approval_link_minted` carries `action` + `target_hash`, and pre-auth there is no user to derive `request_id` from), `client` / `agent_driven` / `user_agent` (the approve-page classifier), `navigation` (true = document navigation, the case Clerk redirects to sign-in; false = a fetch Clerk answers with 404 — agents and scripts). Distinct id is the constant `anonymous-approve-wall`. Added 2026-09-16: a SIGNED-OUT visit to an approval link never renders the page (Clerk's `auth.protect()` redirects first), so the hop had no row and showed up only as "minted, never opened". That hop is what Claude desktop's in-app browser takes on every link click — it holds no FGAC session — and sizing on 2026-09-15 found 15 of 110 never-opened requests (since 09-09) with a sign-in inside the hour, every one landing on the default profile page. Dev-instance note: on localhost the first visit from a cookie-less browser is answered by Clerk's dev-browser handshake BEFORE this handler runs, so the event fires on the post-handshake request; production has no such hop. Deduped per browser: while the marker cookie for the same `action` + `target_hash` is under 5 min old, repeat bounces of that request record nothing (QA 2026-09-16 saw a dev sign-in callback loop re-hit the wall 28 times in three minutes) — so `count()` is wall hits, not bounces, and a person returning after minutes is a new hit. Read human wall hits as `client != 'agent' AND navigation`. Runbook: monitoring.md §7.25 |
| `clerk_auth_redirect` | edge middleware (`src/middleware.ts` outer wrapper → `src/lib/clerkAuthRedirect.ts`, `src/lib/posthogEdge.ts`) | `kind` (`sign_in` = Clerk's hosted sign-in / sign-up page with `redirect_url`, what `auth.protect()`'s thrown control-flow error becomes for a signed-out document navigation; `handshake` = Clerk's frontend-API `/v1/client/handshake`, returned by clerkMiddleware itself when the session state is ambiguous), `path` (the requested path with id-shaped segments replaced by `:id`), `reason` / `auth_status` (Clerk's own `x-clerk-auth-reason` / `x-clerk-auth-status` response headers, e.g. `session-token-expired`, `dev-browser-missing`, `session-token-and-uat-missing`), `client_hash` (16-char keyed hash of the Clerk cookie that identifies the browser — `__session`, else a non-zero `__client_uat`, else the dev-browser token; absent for a cookie-less browser; never the raw value), `browser_key` (`client_hash`, else `bounce_id` — the key the loop query groups on), `has_session_cookie`, `has_client_cookie` (`__client_uat` present and not `0`), `clerk_redirect_count` (Clerk's own per-handshake hop counter cookie; Clerk gives up at 3 and logs its "infinite redirect loop" warning), `client` / `agent_driven` / `user_agent` (the approve-page classifier), `navigation` (document navigation vs fetch, same test as the approval wall), `bounce_id` / `bounce_count` / `bounce_age_s` (the `fgac_clerk_bounce` marker cookie: a random series id, how many Clerk redirects this browser has taken in the current 5-minute window — this hop included — and seconds since the first). Distinct id is the constant `anonymous-clerk-redirect`. Added 2026-09-24: on 2026-09-21 a localhost session bounced between `/dashboard` and Clerk's hosted sign-in for minutes — dozens of Clerk "Refreshing the session token resulted in an infinite redirect loop" warnings in the server log — and PostHog had no row for any of it, because `sign_in_completed` fires only on success and Clerk's warning lives in Vercel's ~1 h runtime log. One row per hop (NOT deduped like `approval_sign_in_wall`: the hop count IS the signal), captured in the outer middleware wrapper where both redirects surface, fire-and-forget via `event.waitUntil`, never touching the response. Dev-instance note: on localhost and previews (dev Clerk instance) every cookie-less browser's first visit takes a `handshake` hop (`reason: dev-browser-missing`) before anything else, so a plain signed-out `/dashboard` visit there records a `handshake` row and then a `sign_in` row — filter `environment = 'production'` for anything that counts loops. Same-pattern sibling of `approval_sign_in_wall` (the wall event stays: it carries the approval join keys this one does not). Runbook: monitoring.md §7.31 |
| `approval_wall_recorded` | server (`POST /api/approval-wall`, called fire-and-forget by middleware on a human wall hit) | `action`, `request_id`, `recorded` (a ledger row existed and took the `wall_hit_at` / `wall_query` stamp). Added 2026-09-16 with post-sign-in routing: the link's key resolves to its owner and the signature is verified against that owner, so the hit is filed under the owner with no session — cross-browser by construction. Only navigations by people (`client != 'agent'`) are recorded; an agent fetching the link is not an owner about to sign in. Joins the wall event by `request_id` → `approval_link_minted` |
| `approval_wall_routed` | server (`/dashboard` page) | `action`, `request_id`, `seconds_since_wall`. Fires when `/dashboard` — where every sign-in lands — sent the owner straight back to an approval whose link hit the sign-in wall in the last 30 min and which they have not opened since; once per wall hit (`routed_at`). Rules: `src/lib/approvalRouting.ts`. This is the repair for the lost-context sign-in: after it ships, `sign_in_completed.after_approval_wall` should fall towards zero, and any remaining rows are people the route did NOT catch (hit older than 30 min, or the ledger had no row — links minted before 2026-08-25). **Read 2026-09-20, four days live: 0 rows in production, and the ledger has no `routed_at` either — not a router defect but nothing for it to do.** Of the 17 walled requests, 16 reached the approve page anyway (Clerk's own `redirect_url` brings a same-browser sign-in straight back; Claude desktop users re-open the link in the browser they are signed in to, 5–95 s later) and 14 were approved. The one dashboard render that met a routable row (a `browser` wall hit with the owner's exact UA, fired 150 ms after that owner's own signed-in open of the same link — a cookie-less duplicate load, not a person) was not routed either; see `monitoring.md` 7.25 for the full read. The durable surface is the pending-approvals banner below. Since 2026-09-21 the "not opened since the hit" rule reads `last_opened_at` (every owner render) rather than the first-open-only `opened_at`, so a request reached again after the hit — through `redirect_url`, the banner or the link — is never routed back to |
| `approval_wall_route_skipped` | server (`/dashboard` page) | `reason` (`skip` = candidate wall-hit rows existed and a rule excluded every one, with per-rule counts `candidates`, `stale`, `routed_already`, `opened_since`, `no_query`, `future`; `lookup_failed` = the ledger read threw). Added 2026-09-20 after a local repro produced one non-route in six with a provably routable row and a clean log: without this row, "no candidate", "rule excluded" and "lookup failed" all read as the absence of `approval_wall_routed`. Not emitted when the owner has no wall-hit rows at all (the common case) |
| `approval_banner_shown` | server (dashboard pages, `src/app/dashboard/PendingApprovalsBanner.tsx`) | `pending_count`, `actions`, `request_ids`, `oldest_pending_s`. One row per dashboard render that had something pending to show (rules: `src/lib/approvalPending.ts` — unapproved, minted in the last 7 d, not dismissed since its last mint, grant not already active, link stored). Renders are frequent (every navigation re-renders), so read owners as `uniq(distinct_id)` and requests through `approval_banner_clicked` / `approval_link_opened {link_source: 'banner'}`, never by counting these rows. Added 2026-09-20: the cookie-free repair for approval context lost between the agent's reply and the dashboard — 47 of 87 requests minted 2026-09-16 → 09-20 were never opened, and their owners found nothing on the dashboard about them |
| `approval_banner_clicked` | client (`PendingApprovalsList.tsx`, the "Review" link) | `request_id`, `action`, `pending_count`. The click; the resulting approve-page render is the `approval_link_opened` row with `link_source: 'banner'` on the same `request_id`, and the grant is the usual `approval_link_approved` |
| `approval_banner_dismissed` | server (dashboard action `dismissPendingApproval`) | `request_id`, `action`. The owner hid the entry; it stays hidden until the agent mints that link again (`dismissed_at` < `last_minted_at`), so a re-request after a dismissal shows a fresh `approval_banner_shown` carrying the same `request_id` |
...
| `approval_link_notified` | server (`src/lib/approvalNotify.ts`, from every mint path) | `channel` (`email`), `trigger` (`repeat_mint`), `action`, `request_id`, `link_count` (2 for a send denial: the per-recipient and send-to-anyone links ride in one email), `mint_count` (how many times the agent had asked), `hours_since_first_mint`. Fires at most once per request: when an agent asks for the SAME link again at least 5 minutes after the first ask and the approve page has never been opened for it, FGAC emails the link from its own support mailbox THROUGH FGAC's own proxy API with the support profile's key (`SUPPORT_FGAC_PROXY_KEY` / `SUPPORT_SENDER_EMAIL`; the send lands as a `proxy_request` row for that key — exclude it from customer counts — and never touches a user's Google grant; that design was rejected 2026-09-15). Capped at 3 per person per rolling 24 h, enforced inside the atomic claim on `approval_requests.notified_at`; since 2026-09-24 the claim runs under a per-owner advisory lock (`src/lib/notifyClaimLock.ts` — the cap counts the owner's OTHER rows, which two concurrent statements could each pass: two reminders 108 ms apart on 09-17) and refuses when another of the owner's requests was emailed inside the 5-minute same-turn window (`skipped_burst` on the mint — two files asked for in one turn are one event, one email). Why: ~half of minted requests were never opened (14 d to 2026-09-13, per request), concentrated on agent surfaces that collapse the tool result. Simulated on 30 d of production mints: 42 emails to 29 people, max 2 to one person in a day. Not a funnel stage: join to `approval_link_minted` on `request_id`; compare opens by `link_source`. Runbook: `monitoring.md` 7.26 |
| `account_refusal_notified` | server (`src/lib/approvalNotify.ts`, from the caller-chosen `account_not_permitted` refusal in `resolveAccountAndToken`) | `channel` (`email`), `trigger` (`account_not_permitted`), `tool`, `refusal_count` (refusals of that value on that key in the rolling 24 h window, ≥ 3), `refusal_count_ever`, `usable_account_count`, `hours_since_first_refusal`. Fires at most ONCE EVER per (proxy key, requested account) AND, since PR #156, at most once per OWNER per 14-day episode whatever the value (a later refused value inside the episode stamps `notify_status: 'skipped_episode'` on the refusal and sends nothing — an agent guessing three wrong addresses earned three emails in 16.7 h on 2026-09-20/21, stopped only by the daily cap): on the 3rd refusal of the same `account` value on one key within 24 h, FGAC emails the owner from the support mailbox (same sender, transport and 3-per-person daily cap as `approval_link_notified` — all three ledgers count toward the cap) naming the value the task passes, the accounts the key can use, and the two fixes (change the task / connect and delegate that account). Why a second trigger: that refusal mints no approval link, so the repeat-mint reminder never fires for it — a scheduled job was refused 53 times in 6 days (2026-09-09 → 09-16) with its owner off the dashboard for 11 days, and the 🚫 copy from PR #137 stopped its retries without changing the task. Never fires for a placeholder value (`account_requested_placeholder` set — example.com, a template, not an address): those refusals skip the ledger entirely and stamp `notify_status: 'skipped_placeholder'`, because an email saying "your task passes ufficio@example.com" told one owner nothing, twice (2026-09-21 — half of all refusal notices sent to date). Ledger: `account_refusals` (one row per key + requested value; `notified_at` is the claim, taken under the owner's advisory lock since 2026-09-24 — the episode rule is an EXISTS over the owner's other rows, and two values refused in one turn both passed it 271 ms apart on 09-21). The email names the agent as `src/lib/agentLabel.ts` renders it (nickname, else "your <client> agent", plus the profile — never an id) and signs as FGAC support with the fixed support contact, never the configured sender. Runbook: `monitoring.md` 7.26d |
| `google_grant_dead_notified` | server (`src/lib/approvalNotify.ts` `notifyOwnerOfDeadGrant`, from the `google_token_unavailable` refusal in `resolveAccountAndToken` and, since 2026-09-25, from the scope pre-flight refusals `gmailScopeDenial` / `driveFileScopeDenial`) | `channel` (`email`), `trigger` (`first_failure` = a dead grant; `scope_missing` = the grant is alive but lacks the scope the tool rides on), `reason` (`grant_revoked` / `refresh_failed` / `no_token` — the reconnect-repairable classes of `google_token_fetch_failed` — or `gmail_scope_missing` / `drive_file_scope_missing`, the `denial_code` of the refusal), `missing_scope` (`gmail` / `drive_file`, scope-missing only — joins to `google_scope_missing.scope`), `account_delegated`, `cc_delegate` (the key owner was CC'd — true exactly when delegated), `failure_count` (refusals in the current episode, 1 unless the sender was off or capped earlier), `days_dead` (whole days since the episode's first refusal; 0 unless the email was delayed by a cap), `via` (`mcp`). **Captured for the mailbox OWNER** (the recipient — for a delegated mailbox that is the grantor, not the person whose agent was refused), so `uniq(person)` is the notified population and the row joins to the owner's own `google_reconnect_*` funnel. Fires **once per (owner, mailbox) episode**: on the first refusal whose class a reconnect repairs, FGAC emails the owner from the support mailbox (CC the key owner when delegated) and never again for that episode; a refusal more than 14 days after the previous one is a new episode (the grant was repaired and died again). Ken, 2026-09-21: one email for one event — the refusal recurs on every agent call, the owner is told once, the CC'd delegate can nudge them, and the dashboard card says "Reconnect Google". Guards: the shared 3-per-person daily cap (all three notice ledgers count) and a global circuit breaker of 10 dead-grant notices per rolling hour across all owners — the classes are deterministic by Clerk error code, so an auth-provider incident would otherwise email every user at once; skipped refusals stamp `notify_status: 'skipped_global_capped'`. Why: in the 30 d to 2026-09-19 one delegated mailbox was refused once a day for 30 days with zero successes and its owner never told (the agent's reconnect link is bound to the owner, and the delegate cannot use it), and two own-mailbox owners got exactly ONE refusal each, never opened the dashboard, and were still dead 48 h later — a second-failure rule would have reached neither, hence first-failure. The three owners who did recover all did so after visiting the dashboard (9 h – 4 d). Never fires from the quiet `list_accounts` probes, from the proxy path (0 failures in 30 d) or for `owner_not_found` / `delegation_inactive` / transient `clerk_error` / `timeout`. Ledger: `google_grant_failures` (one row per owner + mailbox; `notified_at` is the claim, taken under the owner's AND the global advisory lock since 2026-09-24 so the daily cap and the hourly breaker — both counts over other rows — are exact under concurrency). **Scope-missing trigger (2026-09-25):** the same notice, ledger, 3-a-day cap and 10-an-hour breaker now also fire on the FIRST `gmail_scope_missing` / `drive_file_scope_missing` refusal of an episode — measured 7 d to 2026-09-25, nine people were refused for drive.file (22 refusals), two started a reconnect, one verified, none had a later Sheets/Docs success; eight of the nine were launch-cohort accounts with ONE sign-in ever, connected before drive.file joined the sign-in scope set, so the permission had never been asked of them and the agent's link was the only channel. The two scope reasons are one episode class (one email for a mailbox missing both; the reconnect grants both); a class change on the same mailbox — a dead grant reconnected with a box unchecked — starts a new episode, because that reconnect is a second event and the scope email says what the first could not (tick the box: Google leaves a previously declined permission unchecked). Runbook: `monitoring.md` 7.30 (7.30e for the scope split) — the volume watch that keeps this from becoming spam is also step 0.8 of the daily review |
...
| `slides_grant_verification` / `slides_grant_recovered` | server (`/api/rules/verify-slides-access`, approval in `actions.ts`) | slides twins of the sheets grant-funnel events, with `presentation_id` (since 2026-09-17, when Slides became an enforced per-file kind) |
...
| `mcp_input_validation_failed` | server (`/api/mcp`, tee of a 2xx `tools/call` response) | `tool`, `kind` (`invalid_arguments`/`unknown_tool`), `message` (the SDK's text, whitespace-collapsed, ≤300 chars). The SDK's own `isError` -32602 results, which never reach `withToolAnalytics`. Since 2026-09-17 the Zod issue array the SDK embeds in that text is decoded into `first_issue_path` (dotted argument path, e.g. `messageId`, `values.0`, `body`), `first_issue_code` (Zod code: `invalid_type`, `invalid_value` = enum miss, `invalid_union`, `too_small`, …), `first_issue_expected` (type, or the allowed literals joined by `\|`), `first_issue_received` (`undefined` = the required key was absent — with Zod stripping unknown keys, the sent-keys list says what name was used instead), `first_issue_message`, `issue_paths` / `issue_codes` (arrays, ≤10), `issue_count`, `issues_parsed` (false = the text carried no JSON issue array — counted, not decoded), and `sent_keys` / `sent_key_count` (the top-level argument KEYS of the request, never values — from the request envelope, so present on `unknown_tool` rows too). Before 2026-09-17 every row's `message` was cut at `Invalid arguments for tool <name>: [` by the capture regex, so those rows count failures but carry no field (rows to 2026-09-19 00:40Z, when the decoder went live). Since 2026-09-23 the route pre-validates `tools/call` arguments itself after alias normalisation: a failure the SDK would refuse is answered with a guided paragraph instead of the JSON dump and captured from the Zod issues directly with `guided: true` plus the `arg_aliases` props; `message` is then the guided text (capped), never the SDK's. Rows without `guided` after that date are failures the pre-check could not run (JSON-RPC batches, unknown tools). Runbook: `monitoring.md` 7.10 / 7.10a |
| `mcp_auth_attempt` | server (`/api/mcp` `verifyMcpAuth`) | `outcome` (`ok`/`invalid_token`/`no_token`), `client_id`, `strategy_used` (`clerk`/`direct`/`none`), `memo_hit`, `optimizations_enabled`, `success_sample_rate`, `error_class` (Clerk error name, or `audience_mismatch` since 2026-09-16 when a verified token's `aud` claim names another server), `aud_present` (since 2026-09-16: whether the verified token carried `aud` at all — Clerk issues none today, so `false` everywhere until Clerk honours RFC 8707 `resource`; the first `true` is the signal that binding is live), `kid` (on `invalid_token` only), `method`; since 2026-09-10 also `user_agent`, `client_name` (from an unauthenticated `initialize`, when the request was one) and `client_class` / `client_class_signal` (`claude` / `internal` / `scanner` / `direct`, see `classifyMcpClient` in `src/lib/mcpClientSignals.ts`; since 2026-09-12 a `direct` row from a bare runtime UA with no `clientInfo` carries `client_class_signal = 'ua:stock-runtime-no-name'` — unnamed automation, read separately from the unlabelled `direct` rows per monitoring.md 7.21e; since 2026-09-25 a `direct` row from a known third-party product carries `client_class_signal = 'product:grok'` / `'product:cursor'` — a real client family that is not Anthropic's, on its own row in monitoring.md 7.21f) — so a 401 spike is attributable from this event alone; the day after the MCP Registry listing (2026-09-10) registry crawlers and health probes produced ~6× the usual `no_token` volume and every non-probe `invalid_token`, and `client_class = 'scanner'` is what keeps them out of the alert and the funnel (monitoring.md 2–3, 7.5, 7.21). Auth-health substrate for the JWKS/strategy optimizations. **Failures are unsampled; successes are a 1-in-20 per-request sample** — multiply `ok` by 20 for volume, valid only from the 2026-08-25 fix onward (two earlier versions sampled per-token and were biased; see docs/monitoring.md 1). `kid = 'probe'` marks our own synthetic probes, not users |
...
Slides (2026-09-17) follows the same scheme: `slides_grant_age_seconds` /
`slides_grace_retries` / `slides_grace_recovered`, denial codes
`slides_not_exposed` / `slides_blocked` / `slides_read_only`, and
`approval_link_minted.action` ∈ `slides_expose` / `slides_write`. **Raw-API
family rename:** Slides calls through `google_api_get/modify` stamped
`raw_api_family='slides'` while Slides was passthrough (2026-08-19 →
2026-09-17); enforced Slides calls stamp `raw_api_family='presentations'`,
the collection name, exactly as Sheets (`spreadsheets`) and Docs
(`documents`) do — query both values across that boundary. The production
403s on the `slides` family before 2026-09-17 were `SERVICE_DISABLED` at
Google: the Slides API was not enabled on the production GCP project
(727876597677); the dev project (627660126377) has it enabled, which is why
a local run never reproduces them.
...
`agent_connections.client_name` at creation, and renames the row on later
handshakes by the rule in `src/lib/mcpClientName.ts`: an unnamed row (the
opaque `client_id` placeholder — the creating request is in practice never the
initialize POST, so `mcp_connection_created` fires nameless) takes any name,
the directory's connect-time inspector `Anthropic/Toolbox` yields to the first
product name, and after that the most recent product handshake wins (claude.ai
and Claude Code share one registration). Each rename fires
`mcp_connection_client_identified`. The row's name then rides on every
`$mcp_tool_call` (via `requireApproval`) alongside `user_agent` — except that
...
never re-initialize stay opaque. Capturing DCR `client_name` at OAuth
registration remains a possible supplement.
